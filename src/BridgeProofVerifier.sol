...
contract BridgeProofVerifier {
    ...
    function verifyBridgeTransaction(
        ...
        uint256 networkIdentifier
    ) external returns (bool) {
        require(
            proof.commitment > 2 && proof.commitment < FIELD_PRIME - 1,
            "Invalid"
        );
        require(
            proof.responseValue > 2 && proof.responseValue < FIELD_PRIME - 1,
            "Invalid"
        );
        require(
            networkIdentifier > 2 && networkIdentifier < FIELD_PRIME - 1,
            "Invalid"
        );
        require(
            proof.messageHash ==
        ...
        if (verificationTime[msg.sender] != 0) {
            require(
                block.timestamp > verificationTime[msg.sender],
                "Too soon!"
            );
        }

        uint256 challenge = uint256(
            keccak256(abi.encodePacked(proof.commitment, proof.messageHash))
        ) % FIELD_PRIME;
    ...
    function verifyProofComponents(
        ...
        uint256 right = mulmod(
            commitment,
            modExp(SOURCE_BRIDGE_KEY, challenge, FIELD_PRIME),
            FIELD_PRIME
        );
