...
contract Vest {
    ...
    modifier onlyBeneficiary(uint256 vestingId) {
        require(
            vestings[vestingId].beneficiary == msg.sender,
            "Not the beneficiary"
        );
        _;
    }
    ...
    ) external onlyBeneficiary(vestingId) {
        require(
            newBeneficiary != address(0),
            "New beneficiary is zero address"
        );
    ...
    function claimVesting(
        ...
        uint256 claimableAmount = (vesting.totalAmount * availableSteps) /
            totalSteps;
        ...
        require(
            token.transfer(vesting.beneficiary, claimableNow),
