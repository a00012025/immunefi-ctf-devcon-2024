...
contract MultiVoterWallet is Ownable {
    ...
    function propose(
        address target,
        bytes calldata data
    ) external isVoterRegistered(msg.sender) returns (uint256 proposalId) {
    ...
    function vote() external isVoterRegistered(msg.sender) {
        require(proposals.length > 0, "no proposal");
        uint256 proposalId = proposals.length - 1;
        Proposal storage proposal = proposals[proposalId];
        require(
            !proposal.finished &&
                block.timestamp < proposal.voteStart + DURATION,
    ...
    function execute() external {
        ...
        if (proposal.votes * 2 > totalVotes) {
            ...
            require(
                block.timestamp >= proposal.voteStart + DURATION,
                "votes not enough or the proposal isn't finished"
            );
        }
    ...
    function delegate(address delegatee) external {
        Voter storage voter = getVoter(msg.sender);
        address oldDelegatee = voter.delegatee == address(0)
            ? msg.sender
            : voter.delegatee;
    ...
    function _mint(
        address voter,
        uint256 amount
    ) internal notPending perVoterReentrancy(getVoter(voter)) {
